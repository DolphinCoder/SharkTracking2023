...
# Data import
# st = sand tiger, IM = immature, MA = mature, OYO = one year olds, MAM = young of the year, M and F are for sex
...
# Import cleaned data
MAMc <- read.csv(paste(tigerpath, sep = "/", "MAMc.csv"))
...
# Binning MAM - this chunk can be copied with find & replace

### Delaware Bay
# Create T/F vector and attach to MAMc
MAM_coords <- data_frame(MAMc$lon, MAMc$lat)
MAM_del <- pointsInPolygon(MAM_coords, delaware_bay)
# Results in a huge list of true/false nonsense
MAM_del <- as.data.frame(MAM_del)
# m is for mapping
MAMm <- bind_cols(MAMc, MAM_del)
MAMm <- rename(MAMm, Delaware_Bay = MAM_del)


### Delaware Gate
# Create T/F vector and attach to MAMc
MAM_delg <- pointsInPolygon(MAM_coords, delaware_gate)
# Results in a huge list of true/false nonsense
MAM_delg <- as.data.frame(MAM_delg)
# m is for mapping
MAMm <- bind_cols(MAMm, MAM_delg)
MAMm <- rename(MAMm, Delaware_Gate = MAM_delg)


### Duxbury Harbor
MAM_dux <- pointsInPolygon(MAM_coords, duxbury_harbor)
# Results in a huge list of true/false nonsense
Duxbury_Harbor <- as.data.frame(MAM_dux)
# m is for mapping
MAMm <- bind_cols(MAMm, Duxbury_Harbor)
MAMm <- rename(MAMm, Duxbury_Harbor = MAM_dux)


### North Carolina
MAM_nc <- pointsInPolygon(MAM_coords, nc)
# Results in a huge list of true/false nonsense
NC <- as.data.frame(MAM_nc)
# m is for mapping
MAMm <- bind_cols(MAMm, NC)
MAMm <- rename(MAMm, NC = MAM_nc)


### Florida
MAM_fl <- pointsInPolygon(MAM_coords, fl)
# Results in a huge list of true/false nonsense
FL <- as.data.frame(MAM_fl)
# m is for mapping
MAMm <- bind_cols(MAMm, FL)
MAMm <- rename(MAMm, FL = MAM_fl)
...
ggplot(data=east_coast) +
  ...
  coord_fixed(xlim=NCxlim,ylim=NCylim,ratio = 1.3) +
  geom_polygon(data=nc, aes(x=x, y=y), color="black",size=1, fill=NA) +
  geom_point(data=MAMm, aes(x=lon,y=lat, shape = NC), color="blue",size=1)
...
MAMm2 <- make_transit(MAMm) # Run the loop
...
# Data cleaning
MAMb <- select(MAMb, -c("X"))
# *********CHECK THIS LINE IF YOUR DATETIME COLUMN IS EMPTY*******
MAMb$DateandTimeUTC <- as.POSIXct(MAMb$DateandTimeUTC, format = "%m/%d/%Y %H:%M:%OS") # Format may change
# Remove columns I don't care about ("logical nonsense")
MAMb <- select(MAMb, -c(Receiver, Transmitter, BethLocation, Delaware_Bay, Delaware_Gate, Duxbury_Harbor, NC, FL, In_Transit))

# Export MAMb to wherever
